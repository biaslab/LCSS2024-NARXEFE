...
@showprogress for nn in 1:num_exps
    ...
    for k in 1:N
        ...
        # Optimal control
        policy = minimizeQCR(agent, goals, time_limit=tlimit, control_lims=input_lims)
        ...
        NARXsystem.update!(system, u_QCR[k+1])
    end

    save("learningrates/results/learningrate-pol$H-My$My-Mu$Mu-QCR-$nn.jld", "py", py, "y_QCR", y_QCR, "u_QCR", u_QCR, "FE", FE,
